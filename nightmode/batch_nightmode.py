...
def main():
    ...
    if os.path.isfile(resumeDatabaseFile) or os.path.isfile(jobsDatabaseFile):
        ...
        status = (
            "Please re-run {} with <directory of mkvs> ".format(
    ...
    for job in jobList:
        print("({} out of {}) Completed".format(jobList.index(job), len(jobList)))
        replaceNightmode(job)
        if len(resumeJobList) != 0:
            resumeJobList.pop(0)
            createResumeJson(resumeDatabaseFile, resumeJobList, jobsDatabaseFile)
        if job == jobList[-1]:
...
def replaceNightmode(job):
    ...
    if codec == "aac":
        extension = ".m4a"
    nightmodeDownmixFile = "audio-nightmode"
    ...
    for track in job["surroundTracks"]:
        if resume == "copyed-to-source" or resume == "nightmode-drc" + str(
            ...
            writeToFile(
                resumeFile, "extracted-audio" + str(job["surroundTracks"].index(track))
            )
            resume = "extracted-audio" + str(job["surroundTracks"].index(track))
        if resume == "extracted-audio" + str(job["surroundTracks"].index(track)):
            print("Creating Nightmode Downmix Track.")
            nightmode.nightmodeTrack(
                "audio.flac",
                nightmodeDownmixFile + "-" + track[1] + extension,
                False,
                False,
                codec,
            )
            writeToFile(
                resumeFile,
                "nightmode-downmix" + str(job["surroundTracks"].index(track)),
            )
            resume = "nightmode-downmix" + str(job["surroundTracks"].index(track))
        if resume == "nightmode-downmix" + str(job["surroundTracks"].index(track)):
            print("Creating Nightmode Loudnorm Track.")
            nightmode.nightmodeTrack(
                "audio.flac",
                nightmodeLoudnormFile + "-" + track[1] + extension,
                True,
                False,
                codec,
            )
            ...
            resume = "nightmode-loudnorm" + str(job["surroundTracks"].index(track))
        if resume == "nightmode-loudnorm" + str(job["surroundTracks"].index(track)):
            print("Creating Nightmode DRC+Loudnorm Track.")
            nightmode.nightmodeTrack(
                "audio.flac",
                nightmodeDRCFile + "-" + track[1] + extension,
                True,
                True,
                codec,
            )
            writeToFile(
                resumeFile, "nightmode-drc" + str(job["surroundTracks"].index(track))
            )
    ...
    if resume == "nightmode-drc" + str(len(job["surroundTracks"]) - 1):
        ...
        outputMkv(
            ...
            "source.mkv",
            "output.mkv",
            nightmodeDownmixFile,
            nightmodeLoudnormFile,
            nightmodeDRCFile,
            codec,
        )
...
def outputMkv(
    ...
    mkvmergeCmd = ["mkvmerge", "-o", outMkvFile, "--track-order", genTrackOrder(job)]
    mkvmergeCmd += ["--audio-tracks", genKeepAudioTracks(job), inMkvFile]
    for surroundTrack in job["surroundTracks"]:
        trackTitleDownmix = "{} Stereo Nightmode DownmixOnly ({})".format(
...
def extractAudio(mkvFile, trackNum, outFile):
    ffmpegCmd = ["ffmpeg", "-i", mkvFile, "-map", "0:" + trackNum, "-y", outFile]
...
def printTracks(mkvFile):
    ...
    for stream in nightmode.ffprobe(mkvFile)["streams"]:
...
def generateJobList(mkvsPath: str, rejectedFile: str, jobsDatabaseFile: str):
    ...
    for stats in reducedStats:
        ...
        print("Now select tracks that the nightmode tracks will be based on: ", end="")
...
def getFileStats(filelist: list):
    ...
    for file in filelist:
        ...
        for stream in nightmode.ffprobe(filestat["mkv"])["streams"]:
            if stream["codec_type"] == "audio":
                # We only need these
                audioInfo = {}
                audioInfo["index"] = stream["index"]
